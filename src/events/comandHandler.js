...
if (etiket) {
    ...
    if (message.content.includes(uye2)) {
        ...
        message.reply({ embeds: [afkembed.setDescription(`${etiket} üyesi **${reason}** sebebiyle \`${timeObj}\` süresince afk.`)] }).then((e) => setTimeout(() => { e.delete(); }, 10000));
    }
}
if (message.author.id === uye) {
    ...
    message.reply({ content: (`Hey! **AFK** modundan çıkış yaptınız.`)}) .then((e) => setTimeout(() => { e.delete(); }, 10000));
}
