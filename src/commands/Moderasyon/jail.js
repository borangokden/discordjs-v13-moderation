...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (member.roles.cache.get(config.penals.jail.roles)) return message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı önceden cezalandırılmış.`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        if (!message.member.permissions.has("ADMINISTRATOR") && member && member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription("Kendinle aynı yetkide ya da daha yetkili olan birini cezalayamazsın!")] })
        ...
        message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı **${reason}** sebebiyle karantinaya atıldı. 
\`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)] })
        ...
        ${member ? member.toString(): member.username} kullanıcısı ${author} tarafından cezalıya atıldı.
    ...
    db.set(`${cezaID}`, `${member} kullanıcısı ${author} tarafından ${moment(Date.now()).format("LLL")} tarihinde reklam sebebiyle **[KARANTİNA]** cezası almış.`)
