...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (config.penals.jail.limit > 0 && limit.has(author.id) && limit.get(author.id) == config.penals.jail.limit) return message.reply({ embeds: [embed.setDescription("Saatlik sınırına ulaştın!")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        if (member.id === author.id) return message.reply({ embeds: [embed.setDescription("Komutu kendinizde kullanamazsınız.")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        ...
        message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı ${message.author} tarafından karantinadan çıkarıldı.`)] })
