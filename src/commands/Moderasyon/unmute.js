...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    if (!user) return message.reply({ embeds: [embed.setDescription('Geçerli bir kullanıcı belirtmelisin!')] })
    await guild.members.cache.get(user.id).roles.remove(config.penals.mute.roles);
    message.reply({ embeds: [embed.setDescription(`${user} kullanıcısının susturulması ${author} tarafından kaldırıldı.`)] })
