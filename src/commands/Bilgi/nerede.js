...
if(!member) return message.reply({ embeds: [embed.setDescription(`Geçerli bir kullanıcı belirtmelisin.`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    if(!kanal) return message.reply({ embeds: [embed.setDescription(`${member} sesli kanallarda değil.`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
...
kanal.createInvite().then(invite =>
    message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı \`${kanal.name}\` kanalında seste. (bg)
