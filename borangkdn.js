    ...
    if(interaction.isButton()) {
      ...
      if(interaction.customId === "etkinlik") {
        ...
        if(member.roles.cache.has(config.buttons.activity)) {
          ...
          await interaction.reply({ content: `<@&${config.buttons.activity}> rolü başarıyla üzerinizden aldım.`, ephemeral: true });
        } else {
          await member.roles.add(config.buttons.activity);
          await interaction.reply({ content: `<@&${config.buttons.activity}> rolünü üzerinize başarıyla ekledim.`, ephemeral: true });
        };
      };
      ...
      if(interaction.customId === "cekilis") {
        ...
        if(member.roles.cache.has(config.buttons.giveaway)) {
          ...
          await interaction.reply({ content: `<@&${config.buttons.giveaway}> rolü başarıyla üzerinizden aldım.`, ephemeral: true });
        } else {
          await member.roles.add(config.buttons.giveaway);
          await interaction.reply({ content: `<@&${config.buttons.giveaway}> rolünü üzerinize başarıyla ekledim.`, ephemeral: true });
        };
      };
